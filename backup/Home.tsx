...
export default function Home() {
  ...
  const analytics = useMemo(() => {
    ...
    const avgReplyTime = {
      ...
      resolution: average(primaryRows.map((row) => row.resolutionHours)),
    };
            ...
            <article className="glass-card" style={{ gridColumn: 1, gridRow: 1 }}>
              ...
              <ResponsiveContainer width="100%" height={290}>
                <LineChart data={analytics.monthly} margin={{ left: 0, right: 22, top: 16, bottom: 8 }}>
                  ...
                  {/* <Line type="monotone" name="Closed" dataKey="Closed" stroke="#7afa60b8" strokeWidth={3} dot={{ r: 4, fill: "#7afa60b8", strokeWidth: 2 }}>
                    <LabelList dataKey="Closed" position="bottom" fill="#7afa60b8" fontSize={12} />
                  </Line> */}
                </LineChart>
              </ResponsiveContainer>
            </article>

            {/* <article className="glass-card" style={{ gridColumn: 2, gridRow: 6 }}>
              <div className="card-heading" style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "10px" }}> <h3>Average Ticket Response Time</h3> <Activity size={18} /></div>
              <div style={{ display: "grid", gap: "8px", padding: "0 8px 8px" }}>
                {([
                  ...
                  <div key={metric.key} style={{ display: "grid", gap: "2px", textAlign: "center" }}>
                    ...
                    <span style={{ color: "#e2e8f0", fontSize: "12px", fontWeight: 800 }}>{metric.value ? `${metric.value.toFixed(2)} Hour(s)` : "N/A"}</span>
