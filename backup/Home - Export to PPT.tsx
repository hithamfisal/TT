...
/* const exportPerfPpt = (data: any[], monthLabel: string = "All") => {
  try {
    const pptx = new PptxGenJS();

    // Slide 1 — Title
    const slide1 = pptx.addSlide();
    slide1.addText("Monthly Performance Report", {
      x: 1,
      y: 1.5,
      fontSize: 24,
      bold: true,
...
const exportPerfPpt = (data: PerfRow[], monthLabel: string = "All") => {
  try {
    ...
    // Slide 1 — Title
    const slide1 = pptx.addSlide();
    slide1.addText("DMR Monthly Performance Report", { x: 1, y: 1.5, fontSize: 24, bold: true, color: "0a1e3c" });
    slide1.addText(`Month: ${monthLabel}`, { x: 1, y: 2.2, fontSize: 14, color: "666666" });

    // Slide 2 — Table
    const slide2 = pptx.addSlide();
    const tableData = [
      ["S No", "Site Name", "Down Hours", "Availability (Hrs)", "Reliability"],
      ...(data || []).map((r, idx) => [
        String(idx + 1),
        r?.siteName || "N/A",
        String(r?.sitesDownHours || 0),
        String(r?.availHours || 0),
        r?.reliability || "100%"
      ])
    ];

    slide2.addTable(tableData, { x: 0.5, y: 0.5, w: 9, fontSize: 10 });
...
// 2. Tickets Data Exporter
const exportTicketsPpt = (tickets: TicketAggregate[], monthLabel: string = "All") => {
  try {
    const pptx = new PptxGenJS();

    // Slide 1 — Title
    const slide1 = pptx.addSlide();
    slide1.addText("DMR Monthly Tickets Report", { x: 1, y: 1.5, fontSize: 24, bold: true, color: "0a1e3c" });
    slide1.addText(`Target Month: ${monthLabel}`, { x: 1, y: 2.2, fontSize: 14, color: "666666" });
    slide1.addText(`Total Aggregated Tickets: ${tickets.length}`, { x: 1, y: 2.7, fontSize: 12, color: "22d3ee" });

    // Slide 2 — Grid
    const slide2 = pptx.addSlide();
    const tableData = [
      ["S No", "Ticket ID", "Site IDs", "Managed Resource", "Severity", "Issues", "Status"],
      ...(tickets || []).map((t, idx) => {
        const row = t.primary;
        return [
          String(idx + 1),
          t.tt || "N/A",
          Array.from(t.siteIds || []).join(", "),
          row.managedResource || "N/A",
          row.severity || "N/A",
          row.issue || "N/A",
          row.status || "N/A"
        ];
    ...
    slide2.addTable(tableData, { x: 0.2, y: 0.5, w: 9.6, fontSize: 8, colW: [0.5, 1.2, 1.5, 1.5, 1.0, 2.9, 1.0] });
    pptx.writeFile(`DMR_Tickets_Report_${monthLabel.replace(/\s+/g, "_")}.pptx`);
  } catch (error) {
...
import {
  ...
  Layers3,
  Network,
  Presentation, // <-- This is perfectly placed here, do not change it!
  Printer,
  RefreshCw,
  Search,
  ShieldAlert,
      ...
      >
        <nav className="topbar no-print" style={{ marginBottom: "8px" }}>
          <div className="brand-cluster">
            <div className="brand-mark" style={{ whiteSpace: "nowrap" }}>
              <Network size={18} /> DMR Ticketing Dashboard
            </div>
            <PartnerLogoStrip />
          </div>
...
<strong>
  Total Tickets — {exportMonths.length === 0
    ...
    : `${exportMonths.length} months`}
                ...
                <button className="ghost-button" onClick={() => {const currentMonthLabel = exportMonths.length === 1 ? (filterOptions.exportMonthLabels?.[exportMonths[0]] ?? exportMonths[0]) : "All"; exportTicketsPpt(monthlyExportTickets, currentMonthLabel); }}
                ...
                >
                  <Presentation size={16} /> PPT
                      ...
                      <button
  className="ghost-button" // Adjust class to match your style
  onClick={() => exportTicketsPpt(filteredTickets, selectedMonthLabel)}
>
...
<button
  ...
  onClick={() => exportPerfPpt(perfRows, selectedMonthLabel)}
>
  <Presentation size={16} className="text-cyan-400" />
  <span>Export PPT</span>
</button>
...
<section 
  ...
  style={{ 
    ...
    width: "100%",
    display: "grid",
    /* Creates 9 distinct columns: 
       - The search box takes up 1.5 parts of the space
       - The 7 dropdowns take 1 part each, sharing space perfectly
       - The clear button takes up exactly what its text needs (max-content)
    */
...
{/* <StatCard 
  style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", width: "100%" }} 
  label="Root Cause Updated" 
  value={<span style={{ fontSize: "16px", fontWeight: "bold" }}>{analytics.rootCauseUpdated.toLocaleString()}</span>} 
  note={<span style={{ fontSize: "30px", color: "#94a3b8" }}>Tickets with Alarm Root Cause</span>} 
  icon={FileSpreadsheet} tone="#34d399" 
/> */}
...
<StatCard 
  //style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", width: "100%" }} 
  label={<span style= {{ fontSize: "16px",fontWeight: 1000 , color: "#00ff15" }}>{`Repeated RCA Sites`}</span>} 
  value={analytics.repeatedRcaSites.toLocaleString()} note="Site + RCA pairs repeated" 
...
<StatCard 
  ...
  note={`${analytics.rcaNotProvidedCount.toLocaleString()} No RCA`} 
